...
const allBottleShapes = {
  ...
  "500ml_round": {
    ...
    topPath:
      "M0.650391 80.0499C284.683 -26.7291 612.139 -27.0514 895.96 80.0499",
  ...
  "250ml_sweet_box_bottom": {
    ...
    topPath:
      "M7.52186 81.1472 L 381.632 14.2872 C 381.892 14.2272 382.112 14.1972 382.372 14.1972 L 661.272 0.547177 C 663.082 0.463639 664.902 0.485555 666.712 0.657193 L 1034.65 25.4572 C 1037.11 25.6272 1039.55 26.0272 1041.93 26.6472 L 1331.32 84.3272",
    bottomPath:
      "M26.3719 187.597 L 394.142 121.857 C 395.332 121.627 396.552 121.487 397.742 121.427 L 657.172 108.727 C 658.472 108.667 659.812 108.697 661.112 108.817 H 661.222 L 1026.8 133.417 C 1028.64 133.557 1030.49 133.867 1032.24 134.347 C 1032.38 134.377 1032.5 134.407 1032.61 134.437 L 1298.73 187.327",
    topIsReversed: false,
    bottomIsReversed: false,
    modelPath: "./assets/models/250gms_sweet_t.glb",
  ...
  "500ml_sweet_box_bottom": {
    ...
    topPath:
      "M1801.46,112.28L1408.46,34.89a92.5,92.5,0,0,0-11.54-1.5L901.44,0.71A89.54,89.54,0,0,0,891.21,0.6L516.44,18.15a93.51,93.51,0,0,0-11.57,1.3L0.59,108.69",
    bottomPath:
      "M21.28,215.69L527.69,126a76.87,76.87,0,0,1,10.15,-1.13l341.4,-16a78.78,78.78,0,0,1,9,0.09l505.42,33.33a79.13,79.13,0,0,1,10.09,1.33L1770.13,215.81",
    topIsReversed: true,
    bottomIsReversed: false,
    modelPath: "./assets/models/500gms_sweet_t.glb",
    targetMaterials: ["Bottom1"],
...
function syncSweetBoxViewControls() {
  ...
  const bottomRadioDiv =
    document.getElementById("viewBottomRadio").parentElement;

  if (
    (currentShapeType !== "sweet_box" && currentShapeType !== "sweet_box_te") ||
    !currentShape
  ) {
...
function drawWarp(
  ...
  const sliceCount = Math.min(
    sliceCountBase,
    Math.max(1, Math.floor(imgWidth)),
  );
  const sourceSliceWidth = imgWidth / sliceCount;

  const isWideShape =
    shape.type === "round_square" ||
    shape.type === "sweet_box" ||
    shape.type === "sweet_box_te";
  const isSweetBox =
    shape.type === "sweet_box" || shape.type === "sweet_box_te";
  const isLargeRound = [
    "300ml_round",
    "500ml_round",
    "750ml_round",
  ...
  for (let i = 0; i < sliceCount; i++) {
    ...
    const clampedTTop0 = Math.min(
      Math.max(tTop0, -edgeExtension),
      1 + edgeExtension,
    );
    const clampedTTop1 = Math.min(
      Math.max(tTop1, -edgeExtension),
      1 + edgeExtension,
    );
    const clampedTBot0 = Math.min(
      Math.max(tBot0, -edgeExtension),
      1 + edgeExtension,
    );
    const clampedTBot1 = Math.min(
      Math.max(tBot1, -edgeExtension),
      1 + edgeExtension,
    );

    if (
      (clampedTTop0 < -0.05 && clampedTTop1 < -0.05) ||
      (clampedTTop0 > 1.05 && clampedTTop1 > 1.05)
    )
      continue;

    const pTop1 = shape.topSampler.getPointAt(
      Math.min(Math.max(clampedTTop0, 0), 1),
    );
    const pBot1 = shape.bottomSampler.getPointAt(
      Math.min(Math.max(clampedTBot0, 0), 1),
    );
    const pTop2 = shape.topSampler.getPointAt(
      Math.min(Math.max(clampedTTop1, 0), 1),
    );
    const pBot2 = shape.bottomSampler.getPointAt(
      Math.min(Math.max(clampedTBot1, 0), 1),
    );
    ...
    const sliceHeight =
      Math.hypot(midBotX - midTopX, midBotY - midTopY) * heightAdjust;
    const sliceAngle =
      Math.atan2(midBotY - midTopY, midBotX - midTopX) - Math.PI / 2;
    ...
    const sourceRight = Math.min(
      imgWidth,
      Math.ceil((i + 1) * sourceSliceWidth),
    );
    ...
    localCtx.drawImage(
      ...
      sourceW,
      imgHeight,
      -(sliceWidth + overlap) / 2,
      -verticalExtension,
      sliceWidth + overlap,
      sliceHeight + verticalExtension * 2,
    );
    localCtx.restore();
  }
}
  ...
  if (!shape) {
    if (clearBackground)
      localCtx.clearRect(
        0,
        0,
        targetW * (window.devicePixelRatio || 1),
        targetH * (window.devicePixelRatio || 1),
      );
    const dpr = window.devicePixelRatio || 1;
    ...
    localCtx.fillText(
      "No shapes available for this type/view.",
      (targetW * dpr) / 2,
      (targetH * dpr) / 2,
    );
    return;
  ...
  if (isAI) {
    ...
    reader.onload = async function (e) {
      ...
      try {
        ...
        const targetWidth =
          shape && shape.uploadDimensions ? shape.uploadDimensions.width : 3000;
        ...
        img.onload = async () => {
          if (shape && shape.uploadDimensions) {
            if (
              img.width < shape.uploadDimensions.width ||
              img.height < shape.uploadDimensions.height
            ) {
              hideLoading();
              showError(
                "Image Too Small",
                `The rendered AI dimensions (${img.width} × ${img.height}px) are smaller than required.\n\nMinimum size: ${shape.uploadDimensions.width} × ${shape.uploadDimensions.height}px`,
              );
              return;
            }
          }
        ...
        showError(
          "Invalid AI File",
          "The Adobe Illustrator file could not be parsed. Please ensure it was saved with 'Create PDF Compatible File' checked in Illustrator.",
        );
        fileInput.value = "";
        updateUploadPlaceholder();
      }
  ...
  if (isSVG) {
    ...
    reader.onload = async function (e) {
      ...
      if (parserError || !svgElement) {
        ...
        showError(
          "Invalid SVG File",
          "The SVG file could not be parsed. Please make sure it's a valid SVG file.",
        );
        currentImage = null;
        currentImageDataUrl = null;
        textureImageDataUrl = null;
            ...
            : viewBox
              ? (() => {
                  ...
                  return parts.length === 4
                    ? parts[2] / parts[3]
                    : shape.uploadDimensions.width /
                        shape.uploadDimensions.height;
                })()
      ...
      img.onload = async () => {
        if (shape && shape.uploadDimensions) {
          ...
          if (
            originalWidth < expectedWidth ||
            originalHeight < expectedHeight
          ) {
            hideLoading();
            showError(
              "Image Too Small",
      ...
      img.onerror = () => {
        ...
        showError(
          "Failed to Load SVG",
          "The SVG file could not be loaded. Please try a different file.",
        );
        currentImage = null;
        currentImageDataUrl = null;
        textureImageDataUrl = null;
    ...
    reader.onerror = () => {
      ...
      showError(
        "File Read Error",
        "Failed to read the SVG file. Please try again.",
      );
      fileInput.value = "";
      updateUploadPlaceholder();
      mainDraw();
    ...
    reader.onload = async function (e) {
      ...
      img.onerror = () => {
        ...
        showError(
          ...
          "The image file could not be loaded. Please try a different file format (PNG, JPG, etc.).",
        );
        currentImage = null;
        currentImageDataUrl = null;
        textureImageDataUrl = null;
...
async function exportPNG() {
  ...
  try {
    ...
    const targetImgWidth = shape.uploadDimensions
      ? shape.uploadDimensions.width * 2
      : currentImage.width * 2;
    const targetImgHeight = shape.uploadDimensions
      ? shape.uploadDimensions.height * 2
      : currentImage.height * 2;

    let processedImage = currentImage;
    if (
      currentImage.width < targetImgWidth ||
      currentImage.height < targetImgHeight
    ) {
      processedImage = await upscaleImage(
        currentImage,
    ...
    showError(
      "Export Failed",
      "Failed to generate the PNG file. Please try again.",
    );
  }
}
 ...
 */
function envelopeTransformForShape(
  px,
  py,
  srcBounds,
  topPaperPath,
  ...
  if (shape && shape.type !== "rectangle") {
    ...
    const isLargeRound =
      shape.type === "round" &&
      ["300ml", "500ml", "750ml", "1000ml"].some((x) => shape.name.includes(x));
  ...
  function getPtEx(path, offset) {
    if (offset < 0) {
      ...
      return p0 && tan
        ? { x: p0.x + tan.x * offset, y: p0.y + tan.y * offset }
        : p0;
      ...
      return p1 && tan
        ? { x: p1.x + tan.x * diff, y: p1.y + tan.y * diff }
        : p1;
    }
  ...
  const bottomPt =
    getPtEx(bottomPaperPath, bottomOffset) || bottomPaperPath.getPointAt(0);
...
function warpPoint(pt, srcBounds, topPP, bottomPP, topRev, botRev, shape) {
  return envelopeTransformForShape(
    pt.x,
    pt.y,
    srcBounds,
    topPP,
 ...
 */
function adaptiveLineSubdivisions(
  p0,
  p1,
  srcBounds,
  topPP,
  ...
  const leftSubs = adaptiveLineSubdivisions(
    p0,
    mid,
    srcBounds,
    topPP,
  ...
  const rightSubs = adaptiveLineSubdivisions(
    mid,
    p1,
    srcBounds,
    topPP,
 ...
 */
function adaptiveBezierSubdivisions(
  p0,
  cp1,
  cp2,
  p3,
  ...
  for (const t of testTs) {
    ...
    const dev = Math.hypot(
      warpedBezierPt.x - truePt.x,
      warpedBezierPt.y - truePt.y,
    );
...
function warpBezierSegment(
  ...
  const subs = adaptiveBezierSubdivisions(
    p0,
    cp1,
    cp2,
    p3,
...
function warpSinglePaperPath(
  ...
  if (!singlePath || !singlePath.segments || singlePath.segments.length < 1)
    return null;
...
function warpPaperPath(
  ...
  if (paperPath.className === "CompoundPath") {
    ...
    for (const child of paperPath.children) {
      const subD = warpSinglePaperPath(
        child,
        srcBounds,
        topPP,
        bottomPP,
  ...
  if (!paperPath.length || !paperPath.segments || paperPath.segments.length < 1)
    return null;
  return warpSinglePaperPath(
    paperPath,
    srcBounds,
    topPP,
    bottomPP,
    ...
    switch (type) {
      ...
      case "rotate": {
        ...
        if (a.length >= 3) {
          ...
          t = _mmul(
            [1, 0, 0, 1, cx, cy],
            _mmul([c, s, -s, c, 0, 0], [1, 0, 0, 1, -cx, -cy]),
          );
        } else {
          t = [c, s, -s, c, 0, 0];
        }
...
function flattenSVGTransforms(node, parentMatrix, ns, isRoot = false) {
  ...
  if (tag === "g" || tag === "a" || tag === "svg") {
    Array.from(node.children).forEach((c) =>
      flattenSVGTransforms(c, cumM, ns, false),
    );
    return;
  }
  ...
  if (_isIdentity(cumM)) {
    ...
    Array.from(node.children).forEach((c) =>
      flattenSVGTransforms(c, _IDENTITY_M.slice(), ns, false),
    );
    return;
  }
  ...
  if (
    ["rect", "circle", "ellipse", "line", "polyline", "polygon"].includes(tag)
  ) {
    try {
      let pp = null;
      switch (tag) {
            ...
            ...(rx > 0 || ry > 0
              ? {
                  radius: [
                    Math.min(
                      rx,
                      parseFloat(node.getAttribute("width") || "0") / 2,
                    ),
                    Math.min(
                      ry,
                      parseFloat(node.getAttribute("height") || "0") / 2,
                    ),
                  ],
                }
              : {}),
        ...
        case "circle":
          pp = new paper.Path.Circle({
            center: [
              parseFloat(node.getAttribute("cx") || "0"),
              parseFloat(node.getAttribute("cy") || "0"),
            ],
            radius: parseFloat(node.getAttribute("r") || "0"),
          });
          break;
        case "ellipse":
          pp = new paper.Path.Ellipse({
            center: [
              parseFloat(node.getAttribute("cx") || "0"),
              parseFloat(node.getAttribute("cy") || "0"),
            ],
          ...
          break;
        case "line":
          pp = new paper.Path.Line({
            from: [
              parseFloat(node.getAttribute("x1") || "0"),
              parseFloat(node.getAttribute("y1") || "0"),
            ],
            to: [
              parseFloat(node.getAttribute("x2") || "0"),
              parseFloat(node.getAttribute("y2") || "0"),
            ],
          });
          break;
        case "polyline":
        case "polygon": {
          ...
          for (let i = 0; i < pts.length; i += 2)
            sg.push(new paper.Point(pts[i], pts[i + 1]));
      ...
      if (pp) {
        ...
        if (newD) {
          ...
          copyAttrs(node, pathEl, [
            ...(SHAPE_SKIP_ATTRS[tag] || []),
            "transform",
          ]);
      ...
      console.warn("flattenSVGTransforms shape:", tag, e);
    }
    return;
  }
  ...
  if (tag === "text" || tag === "tspan") {
    ...
    if (Math.abs(angle) > 0.01)
      parts.push(`rotate(${warpF(angle)},${warpF(tp.x)},${warpF(tp.y)})`);
    if (Math.abs(sx - 1) > 0.01 || Math.abs(sy - 1) > 0.01)
      parts.push(`scale(${warpF(sx)},${warpF(sy)})`);
...
function warpImageEl(
  ...
  const XLINK_NS = "http://www.w3.org/1999/xlink";
  const hrefVal =
    el.getAttribute("href") || el.getAttributeNS(XLINK_NS, "href") || null;
  ...
  poly.setAttribute(
    "points",
    corners.map((c) => `${warpF(c.x)},${warpF(c.y)}`).join(" "),
  );
    ...
    try {
      ...
      switch (tag) {
        ...
        case "polygon": {
          ...
          const segs = [];
          for (let i = 0; i < pts.length; i += 2)
            segs.push(new paper.Point(pts[i], pts[i + 1]));
      ...
      console.warn("warpNode shape:", tag, e);
    }
    return;
  }
...
function buildWarpedSVG(rawSvgText, shape) {
  ...
  try {
    ...
    if (!srcBounds) {
      ...
      if (wAttr && hAttr) {
        ...
        try {
          ...
          console.warn("Bounds measurement failed:", e);
        }
    ...
    if (isFlatShape) {
      ...
      const drawContent = Array.from(root.children).filter(
        (el) => el.tagName && el.tagName.toLowerCase() !== "defs",
      );

      const g = doc.createElementNS(ns, "g");
      g.setAttribute(
        "transform",
        `scale(${sx}, ${sy}) translate(${-srcBounds.x}, ${-srcBounds.y})`,
      );
    ...
    const drawKids = Array.from(root.children).filter(
...
async function exportSVG() {
  ...
  try {
    ...
    showError(
      "Export Failed",
      "Failed to generate the SVG file. Please try again.",
    );
  }
}
...
async function exportPDF() {
  ...
  try {
    ...
    showError(
      "Export Failed",
      "Failed to generate the PDF file. Please try again.",
    );
  }
}
